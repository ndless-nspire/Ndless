...
local function set_breakpoint(file, line)
  ferr:write(tostring(file)..' '..tostring(line)..'\n')  
...
--when driving with a debugger front-end, there is the opposite problem;
--paths are generally given as absolute, and one must convert as necessary
--However, breakpoint lookup becomes simpler.
local function has_breakpoint(file,line)
...
--    local vars,file,line = capture_vars(level,1,line)
	...
	if rawfile ~= curfile then
        file = rawfile
		curfile = rawfile        
        if strfind(file,"@") == 1 then file = strsub(file,2) end
        if IsWindows then file = strlower(file) end
		if strsub(file,1,1) == '/' or strsub(file,2,2) == ':' then -- an absolute path
			if IsWindows then 
                file = file:gsub('/','\\') -- canonical for Win32!
            end
		elseif project_root_path then -- relative path
    ...
    while true do
      ...
      --print('!!',thisfile,line)      
      if has_breakpoint(thisfile, line) then
        vars = capture_vars(level,1,line)
...
local function debugger_loop(ev, vars, file, line, idx_watch)
  ...
  while true do
    ...
    if command == "setb" or command == 'break' then
      --{{{  set breakpoint
      write(args..'\n')
      local line, filename
