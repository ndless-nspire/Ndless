...
do
    ...
    -- Helper function for string splitting if not available
...
-- Error codes that the payload can return
...
function getInstallerResult()
    ...
    if not installer or type(installer) ~= "string" or #installer < 4 then
        return "install_failed:no_resources"
    end
...
function on.construction()
    ...
    do_install = true  -- fire immediately, no IPC handshake needed
end


function on.timer()
    if not attempted_install and do_install then
        local res = install()
        if res then
            local result = getInstallerResult()
            ipc_send(result)
        else
            ipc_send("install_failed:setup")
        end
